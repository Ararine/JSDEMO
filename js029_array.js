...
console.log(fruits.push("Melon")); //6 /새로운 요소를 추가하고 배열의 길이를 리턴
...
//javascript 에서 sort() 는 Tim Sort 알고리즘을 사용한다.
//Tim Sort 는 Insertion sort 와 Merge Sort 을 결합하여 만든 정렬이다.
//Tim Sort 알고리즘 : https://d2.naver.com/helloworld/0315536
let point = [40, 100, 1, 5, 10];
console.log(point); //[ 40, 100, 1, 5, 10 ]
...
// a.concat(b) 배열을 합쳐서 리턴
let xData = [1, 2, 3];
let yData = [4, 5, 6];
let zData = xData.concat(yData, [7, 8, 9]); // 인자값으로 배열 추가 가능
console.log(zData); //[ 1, 2, 3, 4, 5, 6, 7, 8, 9 ]
console.log(zData.length); //9

//flat() 배열 안에 배열을 하나씩 벗겨준다.
let arr2 = [1, 3, [[[4, 5]]], [7, 8], [[9, 10]], 12, [13, [14]]];
